...
| 項目 | 成果 |
|---|---|
| 子集化 Iansui 標題字型（`scripts/subset-fonts.sh`） | 4.9MB → 231KB woff2 |
| 下載酬載 | ~7MB → ~615KB |
...
### 2. 移除或瘦身 GenSenRounded（低風險清理）
`public/fonts/GenSenRounded2TW-R.otf` 為 16MB、`font-display: optional`，弱網不會下載，但仍佔關鍵鏈 ~2s，且讓部署產物多 16MB。

- **選項 A（移除）**：拿掉 `@font-face` 與 `--font-sans` 中的 `GenSenRounded`，內文改用 `Noto Sans TC`。注意：桌機/重訪會從圓體變成 Noto Sans（細微視覺差異）。
- **選項 B（瘦身）**：比照 Iansui 用 `scripts/subset-fonts.sh` 子集成 woff2（~數百 KB），讓圓體在行動裝置也能用。
- **影響**：移除關鍵鏈那 2s + 部署 16MB；分數**不一定**會動（因主要閘門是 JS 渲染）。
